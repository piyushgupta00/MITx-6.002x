    ...
    def update(self):
        ...
        for index in range(len(self.viruses)):
            ...
            if self.viruses[index].doesClear() ==  True:
                ...
                curr_pop_density= float(len(curr_virus_list)/self.getMaxPop())
                try:
                   curr_virus_list.append(self.viruses[index].reproduce(curr_pop_density,self.druglist))
                except NoChildException :
                   continue
        self.viruses =  curr_virus_list
        return len(self.viruses)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    #treated_patient2 = treated_patient.copy()
    ...
    for trials in range(numTrials):
        ...
        #virus_growth_list = np.array(virus_growth_list)
        avg_virus_pop = [avg_virus_pop[i] + total_virus_growth_list[i] for i in range(len(total_virus_growth_list))]
        resist_virus_pop = [resist_virus_pop[i] + resist_virus_growth_list[i] for i in range(len(resist_virus_growth_list))]
        
    y_axis_total = [x/numTrials for x in avg_virus_pop]
    y_axis_resist = [x/numTrials for x in resist_virus_pop]
    
    # a drug added to the virus ecosystem
    #reated_patient.addPrescription('guttagonol')
