...
def generate_models(x, y, degs):
    ...
    degrees = degs
    models = []
    #print('xval length:',len(xvals))
    #print('yval length:',len(yvals))
    for degree in degrees:
        models.append(np.polyfit(xvals,yvals,degree))
    return models
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        for item in range(len(x)):
          yEst.append(np.polyval(model, x[item]))
          
        model_yEst.append(yEst)
        model_performance[tuple(model)] = r_squared(y,yEst)
    
    for i in range(len(models)):
        plt.figure(i)
        plt.plot(x, y, 'bo', x, model_yEst[i], 'r')
        plt.xlabel('Years')
        plt.ylabel('Daily Mean High')
     
        plt.title('Temp Variance of degree '+str(i+1)+',' '(R^2)= '+str(round(r_squared(y, model_yEst[i]), 4)))
